...
restored to `automations.yaml`, and the scripts to `scripts.yaml`.
...
3. Add both files from `scripts/` to `scripts.yaml` and the automations to
   `automations.yaml`.
4. Validate the configuration before restarting.
5. Import `dashboards/ev_energy.yaml` or recreate the cards manually.
